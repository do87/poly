...
func (h *handler) setRoutes() *handler {
	...
	h.route.Post("/agents/key", h.keys.create(h.agents.uc))        // create a new agent key
	h.route.Delete("/agents/key/{id}", h.keys.delete(h.agents.uc)) // Delete an agent key by ID
	return h
}
